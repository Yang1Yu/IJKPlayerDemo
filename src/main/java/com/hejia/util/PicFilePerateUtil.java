...
/**
 * 文件名：PicFilePerateUtil
 * 作者：韩秋宇
 * 时间：2017/5/19
 * 功能描述：读取usb设备的图片文件的工具类
 */
public class PicFilePerateUtil implements FileFilter {
     ...
     * 获取文件的路径
     */
    public List<String> pathFile(String path){
        List<String> imgList = new ArrayList<>();
        List<String> allFileList = getAllFile(path);
        for(int i=0;i<allFileList.size();i++){
            imgList.add(path+"/"+allFileList.get(i));
        }
        return imgList;
    }
}
